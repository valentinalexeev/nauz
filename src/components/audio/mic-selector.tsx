...
export function MicSelector({
  ...
  const currentDevice = devices.find((d) => d.deviceId === selectedDevice) ||
    devices[0] || {
      label: loading ? "Загрузка..." : "Микрофон не найден",
      deviceId: "",
    };
  ...
  return (
    <DropdownMenu onOpenChange={handleDropdownOpenChange}>
      ...
      <DropdownMenuContent align="center" side="top" className="w-72">
        {loading ? (
          <DropdownMenuItem disabled>Загружаем устройства...</DropdownMenuItem>
        ) : error ? (
          <DropdownMenuItem disabled>Ошибка: {error}</DropdownMenuItem>
        ) : (
          devices.map((device) => (
            <DropdownMenuItem
              key={device.deviceId}
...
export function useAudioDevices() {
  ...
  const loadDevicesWithoutPermission = useCallback(async () => {
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Не удалось получить список устройств",
      );
